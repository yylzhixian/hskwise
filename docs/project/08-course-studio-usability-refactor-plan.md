...
- 新增 Phase U（Usability），按 U0-U4 分阶段实施。
- 只有 U4 的产品验收门全部通过后，才能恢复 Phase 5。
...
- [ ] U1-02 将 Studio 壳层、课程结构、模板入口翻译为简体中文。
- [ ] U1-03 分离 UI 语言与内容语言。
- [ ] U1-04 重做顶部栏，收纳导入、导出和开发工具。
- [ ] U1-05 新增课程结构、内容编辑、动画时间轴、预览检查模式。
- [ ] U1-06 将 mode、scene 和主要 selection 同步到 URL。
- [ ] U1-07 建立 `CourseStudioProvider` 和 Travels 文档状态。
- [ ] U1-08 将 Shell 的 Scene CRUD 迁移到命令层。
- [ ] U1-09 实现撤销、重做按钮和键盘操作。
- [ ] U1-10 实现 v1 草稿到 Travels v2 snapshot 的迁移。
