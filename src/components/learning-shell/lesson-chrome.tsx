...
export function LessonChrome({
  ...
  return (
    <LearningAppearanceRoot className="flex min-h-dvh flex-col bg-background pb-[calc(5rem+env(safe-area-inset-bottom))] text-foreground">
      ...
      {reserveFeedbackSpace || feedback ? (
        <div
          aria-live="polite"
          className={cn(
            'fixed inset-x-0 bottom-[calc(5rem+env(safe-area-inset-bottom))] z-30 mx-auto flex min-h-22 w-full max-w-3xl items-center px-4 py-3 sm:px-6',
            !feedback && 'invisible'
          )}
        >
          {feedback}
      ...
      <footer className="fixed inset-x-0 bottom-0 z-40 border-t bg-background/90 pb-[env(safe-area-inset-bottom)] backdrop-blur-md">
